...
function search(query: string): SearchResult[] {
    ...
    const topicResults: SearchResult[] = networkTopics
        .map((t) => ({
            s: Math.max(
                score(t.title, q) * 4,
    ...
    const glossaryResults: SearchResult[] = glossary
        .map((g) => ({
            s: Math.max(score(g.term, q) * 4, ...(g.aliases ?? []).map((a) => score(a, q) * 3), score(g.definition, q)),
            g,
        }))
        .filter((x) => x.s > 0)
        .sort((a, b) => b.s - a.s)
    ...
    const sectionResults: SearchResult[] = sectionIndex
        ...
        .slice(0, 4)
        .map((s) => {
            const topic = networkTopics.find((t) => t.id === s.topicId)
            return {
