    ...
    return (
        <div className="max-w-4xl mx-auto px-6 py-10 space-y-14">
            ...
            <Section id="s111" title="11.1  Netfilter 구조">
                ...
                <InfoTable headers={['Hook', '위치', '주요 용도']} rows={hookRows} />
            ...
            {/* ── 11.2 ──────────────────────────────────────────────── */}
            <Section id="s112" title="11.2  iptables와 nftables">
                <Prose>
                ...
                <InfoTable headers={['항목', 'iptables', 'nftables']} rows={iptablesVsNftRows} />
            ...
            <Section id="s113" title="11.3  conntrack과 stateful 방화벽">
                ...
                <InfoTable headers={['상태', '설명', '예시']} rows={conntrackStateRows} />
                <CardGrid cols={2}>
                    <StatCard title="기본 conntrack 테이블 크기" value="65,536" color="cyan" desc="nf_conntrack_max 기본값. 대규모 서버는 131072+ 권장" />
                    <StatCard title="TCP ESTABLISHED 타임아웃" value="432,000초" color="amber" desc="5일 (nf_conntrack_tcp_timeout_established)" />
                </CardGrid>
            ...
            <Section id="s114" title="11.4  SNAT / DNAT / MASQUERADE">
                ...
                <InfoTable headers={['유형', '체인', '동작', '사용 사례']} rows={natCompareRows} />
                <CardGrid cols={2}>
            ...
            {/* ── 11.5 ──────────────────────────────────────────────── */}
            <Section id="s115" title="11.5  포트 포워딩">
                <Prose>
                    포트 포워딩(Port Forwarding)은 DNAT의 대표적인 활용 사례입니다.
                    외부에서 특정 포트로 들어오는 트래픽을 내부 네트워크의 다른 호스트/포트로 전달합니다.
                </Prose>
            ...
            {/* ── 11.6 ──────────────────────────────────────────────── */}
            <Section id="s116" title="11.6  mark / fwmark과 policy routing">
                <Prose>
                ...
                <CardGrid cols={3}>
                    <StatCard title="mark 크기" value="32-bit" color="purple" desc="0x00000000 ~ 0xFFFFFFFF" />
                    <StatCard title="적용 테이블" value="mangle" color="orange" desc="MARK 타겟으로 설정" />
                    <StatCard title="라우팅 연동" value="ip rule" color="teal" desc="fwmark 조건으로 테이블 선택" />
                </CardGrid>
            ...
            {/* ── 11.7 ──────────────────────────────────────────────── */}
            <Section id="s117" title="11.7  TPROXY와 transparent proxy">
                <Prose>
                    Transparent Proxy는 클라이언트가 프록시의 존재를 인식하지 못하는 상태에서
                    트래픽을 프록시 서버로 리다이렉트하는 방식입니다.
                </Prose>
                <CardGrid cols={2}>
            ...
            {/* ── 11.8 ──────────────────────────────────────────────── */}
            <Section id="s118" title="11.8  inline vs out-of-path 장비">
                <Prose>
                    네트워크 보안/모니터링 장비는 트래픽 경로 상의 위치에 따라
                    inline(인라인)과 out-of-path(아웃오브패스)로 나뉩니다.
                </Prose>
                <InfoTable headers={['항목', 'Inline (인라인)', 'Out-of-path (아웃오브패스)']} rows={inlineVsOopRows} />
                <CardGrid cols={2}>
            ...
            <Section id="s119" title="11.9  요약">
                <CardGrid cols={2}>
                    ...
                    <InfoBox color="purple" title="iptables vs nftables">
                        ...
                        nftables는 set/map으로 O(1) 매칭이 가능합니다.
                    </InfoBox>
                ...
                <Alert variant="tip" title="다음 단계:">
                    Topic 12에서는 RSS/RPS, qdisc, XDP, DPDK 등 고성능 패킷 처리와
